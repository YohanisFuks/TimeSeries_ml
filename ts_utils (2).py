...
def find_outliers_prophet(ts, perc=0.01):
    ## fit svm
    scaler = preprocessing.StandardScaler()
    ts_scaled = scaler.fit_transform(ts.values.reshape(-1,1))
    model = svm.OneClassSVM(nu=perc, kernel="rbf", gamma=0.01)
...
'''
Interpolate outliers without ploting
'''
def remove_outliers_prophet(ts, outliers_idx):
    ts_clean = ts.copy()
    ts_clean.loc[outliers_idx] = np.nan
    #ts_clean = ts_clean.interpolate(method="linear")
    #ax = ts.plot(figsize=figsize, color="red", alpha=0.5, title="Remove outliers", label="original", legend=True)
...
'''
Build TS dataframe full.
create data range from the first day to the last day
fill the ts information with the ma7
fill the ts information with linear interpolation if needed
'''
def build_ts_df(ts):
    length = len(ts)
    first_day = ts.index[0]
    last_day = ts.index[-1]
    df_ts = pd.DataFrame({'DATA' : pd.date_range(start = first_day, end = last_day, freq = 'D'),
                         'zeros':0})
    df_ts.set_index(['DATA'], inplace = True)
    df_ts = pd.concat([df_ts, ts], axis = 1).drop(columns = 'zeros')
    df_ts = df_ts.assign(ma7 = lambda x: x.ts.rolling(7, min_periods = 1).mean(),
                        ts_original = lambda x: x.ts)
    df_ts.ts.fillna(df_ts['ma7'], inplace = True)
    df_ts.ts.interpolate(method = 'linear', inplace = True)
    return df_ts
...
def base_feriados():
    '''
    Build TS with BR holidays full.
    '''
    feriados_carnaval = pd.DataFrame( data = {'datas': ['2015-02-16','2015-02-17','2015-02-18',
                                                          '2016-02-08','2016-02-09','2016-02-10',
                                                          '2017-02-28','2017-02-27','2017-03-01',
...
#black friday
    feriados_black_friday =  pd.DataFrame(data = {'datas'  : ['2015-11-27',
                                                             '2016-11-25',
                                                             '2017-11-24',
                                                             '2018-11-23',
                                                             '2019-11-29',
...
#ano novo!!
    feriados_natal_anonovo = pd.DataFrame(data = {'datas' : ['2015-12-25', '2015-12-31', '2016-01-01',
                                                              '2016-12-25', '2016-12-31', '2017-01-01',
                                                              '2017-12-25', '2017-12-31', '2018-01-01',
                                                              '2018-12-25', '2018-12-31', '2019-01-01',
                                                              '2019-12-25', '2019-12-31', '2020-01-01',
                                                              '2020-12-25', '2020-12-31', '2021-01-01',
                                                              '2021-12-25', '2021-12-31', '2022-01-01'],
                                               'flag_virada_ano': 1})


#papitos
    feriados_pais = pd.DataFrame(data = {'datas': ['2017-05-14','2017-06-12', '2017-08-13','2017-10-12',  ##dia das maes// dia dos namorados // dia dos pais // dia das crianças
                                                     '2018-05-13','2018-06-12', '2018-08-12','2018-10-12',  ##dia das maes// dia dos namorados // dia dos pais // dia das crianças
                                                     '2019-05-12', '2019-06-12', '2019-08-11', '2019-10-12', ##dia das maes// dia dos namorados // dia dos pais // dia das crianças
                                                     '2020-05-10', '2020-06-12', '2020-08-09', '2020-10-12', ##dia das maes// dia dos namorados // dia dos pais // dia das crianças
                                                     '2021-05-09', '2021-06-12', '2021-08-08', '2021-10-12' ##dia das maes// dia dos namorados // dia dos pais // dia das crianças
                                                     ],
                                     'flag_pais' : 1})


#el coelho
    feriados_pascoa = pd.DataFrame(data = {'datas':['2017-04-16',
                                                       '2018-04-01',
                                                       '2019-04-21',
                                                       '2020-04-12',
                                                       '2021-04-04'],
    ...
    feriados_genericos = pd.DataFrame(data = {'datas':['2017-04-14','2017-04-21','2017-05-01', '2017-06-15', #sexta feira santa // tiradantes // 1 maio // corpus christi
                                                          '2017-09-07','2017-11-02','2017-11-15', '2017-11-20',  #7set //  finados // republica // consciencia negra
                                                         
                                                           '2018-03-30','2018-04-21','2018-05-01','2018-05-31', #sexta santa // tiradentes // 1 de maio // corpus christi
                                                          '2018-09-07', '2018-11-02', '2018-11-15', '2018-11-20',  #7set //  finados // republica // consciencia negra
                                                          
                                                          '2019-04-19','2019-04-21','2019-05-01','2019-06-20', #sexta santa // tiradentes // 1 de maio // corpus christi
                                                          '2019-09-07', '2019-11-02', '2019-11-15', '2019-11-20',  #7set //  finados // republica // consciencia negra
                                                          
                                                          '2020-04-10','2020-04-21','2020-05-01','2020-06-11', #sexta santa // tiradentes // 1 de maio // corpus christi
                                                          '2020-09-07', '2020-11-02', '2020-11-15', '2020-11-20',  #7set //  finados // republica // consciencia negra
                                                          
                                                          '2021-04-02','2021-04-21','2021-05-01','2021-06-03', #sexta santa // tiradentes // 1 de maio // corpus christi
                                                          '2021-09-07', '2021-11-02', '2021-11-15', '2021-11-20'  #7set //  finados // republica // consciencia negra
                                                   ],
                                          'flag_gen': 1})


    df_feriados = pd.concat([feriados_black_friday,feriados_carnaval,feriados_genericos,feriados_natal_anonovo,feriados_pais,feriados_pascoa])
    df_feriados['datas'] = df_feriados['datas'].astype('datetime64')
    return(df_feriados)
...
def df_forecast_frame(df, horizonte):
    '''
    Build forecast TS with BR holidays full.
    '''    
    df_forecast = pd.DataFrame(pd.date_range(start = df.index.max() + timedelta(days = 1) , periods =  horizonte , freq = 'D'), columns = ['PAY_MOVE_DATE'])
    
  
    feriados = base_feriados()
    df_forecast = pd.merge(feriados, df_forecast, how = 'right', left_on = 'datas', right_on = 'PAY_MOVE_DATE').drop(columns =    'datas').sort_values('PAY_MOVE_DATE')
    df_forecast.loc[:,'flag_black_friday':'flag_virada_ano'] = np.where(pd.isnull(df_forecast.loc[:,'flag_black_friday':'flag_virada_ano']),0,1 )


    return df_forecast.iloc[:horizonte, ]
...
#model 1
def sarima_sem_exo_0(df, horizonte):
    model1 = auto_arima(df['ts'],start_p = 1, 
                   start_q = 1,
                   max_p = 7, 
                   max_q = 7,
                   max_P = 3,
                   ...
                   maxiter = 300,
                   scoring = 'mse',
                   out_of_sample_size = horizonte,
                   n_jobs = -1,
                   #exogenous= df_final.iloc[:,-6:],
                   suppress_warnings=True,
                   stepwise=False)
    return model1

#model 2
def sarima_sem_exo_1(df, horizonte):
    model2 = auto_arima(df['ts'],start_p = 1, 
                   start_q = 1,
                   max_p = 7, 
                   max_q = 7,
                   max_P = 3,
...
#model 3
def sarima_com_exo_0(df, horizonte):
    model3 = auto_arima(df['ts'],start_p = 1, 
                   start_q = 1,
                   max_p = 7, 
                   max_q = 7, 
                   max_P = 3,
...
#model 4
def sarima_com_exo_1(df, horizonte):
    model4 = auto_arima(df['ts'],start_p = 1, 
                   start_q = 1,
                   max_p = 7, 
                   max_q = 7, 
                   max_P = 3,
...
def feriados_prophet():
    feriados = base_feriados()
    black_friday = pd.DataFrame({'holiday':'black_friday',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_black_friday'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    carnaval = pd.DataFrame({'holiday':'carnaval',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_carnaval'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    genericos = pd.DataFrame({'holiday':'genericos',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_gen'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    virada_ano = pd.DataFrame({'holiday':'virada_ano',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_virada_ano'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    pais = pd.DataFrame({'holiday':'pais',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_pais'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    pascoa = pd.DataFrame({'holiday':'pascoa',
                                 'ds': pd.to_datetime(feriados.loc[feriados['flag_pascoa'] == 1,'datas']),
                                 'lower_window' : 0 ,
                                 'upper_window' : 1
    })

    holidays = pd.concat([black_friday, carnaval, genericos, virada_ano, pais, pascoa])
    return holidays